...
void get_erdos(PublData* data, const int64_t id1, const int64_t id2, int64_t* verified, int distance, int* answer) {
    ...
    while (curr) {
        ...
        for (i = 0; i < paper->num_authors; i++) { 
            ...
            get_erdos(data, paper->author_ids[i], id2, verified, distance + 1, answer);
        }
        curr = curr->next;
    }
}
...
int get_number_of_authors_with_field(PublData* data, const char* institution,
    ...
    int a, x, i, j, nr = 0;
    int64_t *verified;
    verified = calloc(100,  sizeof(int64_t));
    struct LinkedList *field_list = get_field_list(data, field);
    struct Node *curr = field_list->head;
    while (curr) {
        struct paper_data *paper_data = curr->data;
    ...
    while (curr) {
        ...
        for (i = 0; i < paper_data->num_authors; i++) {
            if (!memcmp(&paper_data->author_ids[i], &id_author, sizeof(int64_t))){
                struct LinkedList* cit_list = get_cit_list(data, &paper_data->id);
                if (cit_list) {
